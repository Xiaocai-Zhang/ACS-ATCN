...
from evaluate.multi.evaluation_val import Evaluate
import numpy as np
import random
import pathlib
...
class config:
    ...
    TrainInpPath = '../../data/proced/UWaveGestureLibraryAll/train_input.csv'
    TrainOupPath = '../../data/proced/UWaveGestureLibraryAll/train_target.csv'
    ParaSavePath = '../../para/UWaveGestureLibraryAll/parameter.csv'
    ModelSavePath = '../../save_model/UWaveGestureLibraryAll/'

    bounds_cost = [(0, 1),(1,10),(1,20)]
    # batch, timestep, filter, layer, kernel, dropout, lr
    bounds_other = [(10, 300),(10, 945),(1, 100),(1, 10),(1, 100),(0,1),(0.001, 0.1)]
...
def Train(x,round_no,f):
    ...
    ratio2 = round(x[1], 4)
    ratio3 = round(x[2], 4)
    cost_2 = round(cost_1 * ratio2, 4)
    cost_3 = round(cost_1 * ratio3, 4)
    batchsize = int(round(x[3]))
    timestep = int(round(x[4]))
    channel = int(round(x[5]))
    layer = int(round(x[6]))
    kernel_tcn = int(round(x[7]))
    dropout = round(x[8], 4)
    lr = round(x[9], 4)
    ...
    LastIdx = [idx for idx in range(945)]
    ...
    printtxt = "G_mean: %.4f: %s,%s,%s,%s,%s,%s,%s,%s,%s,%s"%(G_mean_val,cost_1,ratio2,ratio3,batchsize,timestep,channel,layer,kernel_tcn,dropout,lr)
    print(printtxt)
...
def RunDE(round_no):
    ...
    SaveTextPath = '../../text/UWaveGestureLibraryAll/round_' + str(round_no) + '/'
